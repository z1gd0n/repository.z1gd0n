...
def maintMenu(view=None):
	...
	feq        = ['Weekly', 'Daily', '3 Days', 'Always']
...
def saveMenu():
	...
	#addFile('Install Movie Wall: %s' % wall.replace('true',on).replace('false',off)                   ,'togglesetting',
	#'keepmoviewall',    icon=ICONTRAKT, themeit=THEME1)
	addFile('Save Skin Shortcuts: %s' % skin.replace('true',on).replace('false',off)               ,'togglesetting', 'keepskin',    icon=ICONTRAKT, themeit=THEME1)
	...
	if type == 'gui':
		...
		if yes_pressed:
			...
			if 'google' in buildzip:
			  ...
			  downloader.download(buildzip, lib, DPwiz.checkBuild(name, 'url'))
			xbmc.sleep(500)
...
def buildVideo(name):
	wiz.log("DDDDDDDDDDDDDDDDDDDDDDDDD: %s" % wiz.workingURL(BUILDFILE))
	if wiz.workingURL(BUILDFILE) == True:
	...
	if yes_pressed:
		...
		for root, dirs, files in os.walk(xbmcPath,topdown=True):
			...
			for name in files:
				...
				if fold[x-2] == 'userdata' and fold[x-1] == 'addon_data' and 'skin' in fold[x]  and KEEPSKIN =='true' : wiz.log("Keep Skin: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
				elif name == 'MyVideos99.db' and name== 'MyVideos107.db' and fold[x-2] == 'userdata' and fold[x-1] == 'Database' and KEEPMOVIEWALL =='true': wiz.log("Keep Movie Wall: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
			...
			for name in dirs:
				...
				if name not in ["Database","userdata","temp","addons","addon_data"]:
				 if not (name == 'script.skinshortcuts' and KEEPSKIN =='true'):
				   	if not (name == 'script.keepmoviewall' and KEEPMOVIEWALL=='true'):

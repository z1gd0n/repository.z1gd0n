...
def checkBuild(name, ret):
	...
	filesize='587'
	if 'Filesize' in link:
	 match = re.compile('name="%s".+?ersion="(.+?)".+?rl="(.+?)".+?ui="(.+?)".+?odi="(.+?)".+?heme="(.+?)".+?con="(.+?)".+?anart="(.+?)".+?review="(.+?)".+?dult="(.+?)".+?escription="(.+?)".+?ilesize="(.+?)"' % name).findall(link)
	 if len(match) > 0:
		for version, url, gui, kodi, theme, icon, fanart, preview, adult, description,filesize in match:
			...
			elif ret == 'filesize':      return filesize

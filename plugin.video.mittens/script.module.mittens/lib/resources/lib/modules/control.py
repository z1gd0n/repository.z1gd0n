...
def artPath():
    theme = appearance()
    if theme in ['-', '']: return
    elif condVisibility('System.HasAddon(script.mittens.artwork)'):
        return os.path.join(xbmcaddon.Addon('script.mittens.artwork').getAddonInfo('path'), 'resources', 'media', theme)


def appearance():
    appearance = setting('appearance.1').lower() if condVisibility('System.HasAddon(script.mittens.artwork)') else setting('appearance.alt').lower()
    return appearance


def artwork():
    execute('RunPlugin(plugin://script.mittens.artwork)')
...
def metaFile():
    if condVisibility('System.HasAddon(script.mittens.metadata)'):
        return os.path.join(xbmcaddon.Addon('script.mittens.metadata').getAddonInfo('path'), 'resources', 'data', 'meta.db')
